...
const API_BASE = import.meta.env.VITE_API_URL || (import.meta.env.DEV ? '' : 'https://kidrove-workshop-g20f.onrender.com');

// ─── Types ─────────────────────────────────────────────────────────────────
interface TimeLeft  { days: number; hours: number; minutes: number; seconds: number }

// ─── Utils ──────────────────────────────────────────────────────────────────
// (Removed unused sanitize, EMAIL_RE, PHONE_RE since DynamicForm handles it)
...
// ─── Confetti ───────────────────────────────────────────────────────────────
// (Moved to DynamicForm or omitted)
...
export default function WorkshopLandingPage() {
  ...
  const selectedWorkshop = workshops.find(w => (w.id || w._id) === selectedWorkshopId);
        ...
        <div className="max-w-7xl mx-auto px-5 py-3.5 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-7 text-sm font-semibold text-gray-500">
            ...
            <button id="nav-enroll-btn" onClick={scrollToForm}
              ...
              Enroll Now {selectedWorkshop ? `— ₹${selectedWorkshop.price}` : ''}
            </button>
          </nav>
          ...
          <div ref={heroRightRef} className="reveal reveal-right delay-200 space-y-4">
            ...
            <div className="glass rounded-3xl shadow-card-hover p-7 space-y-5 border border-white/60">
              ...
              <div className="space-y-1">
                {[
                  { label: 'Start Date', val: selectedWorkshop?.dates || '15 July 2026',        color: 'text-orange-600',           icon: <Calendar    className="h-4 w-4" /> },
                  ...
                  { label: 'Batch Size', val: `Max ${selectedWorkshop?.capacity || 20} Students`,      color: 'text-sky-600',              icon: <Users       className="h-4 w-4" /> },
                  { label: 'Fee',        val: `₹${selectedWorkshop?.price || 2999} (Early Bird)`,   color: 'text-emerald-600 font-extrabold', icon: <CreditCard  className="h-4 w-4" /> },
              ...
              <div className="bg-white rounded-3xl border border-gray-100 shadow-card-hover overflow-hidden">
                ...
                <div className="relative bg-gradient-to-br from-violet-700 via-violet-600 to-purple-700 px-7 py-7 text-white overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    {workshops.length > 0 && (
                      <div className="mt-3">
                        <label className="block text-xs font-bold text-violet-200 mb-1 uppercase tracking-wider">Select Workshop</label>
                        <select
                          value={selectedWorkshopId}
                          onChange={(e) => setSelectedWorkshopId(e.target.value)}
                          className="w-full bg-white/10 border border-white/20 text-white text-sm rounded-lg p-2.5 focus:outline-none focus:border-white/40 [&>option]:text-gray-900"
                        >
                          {workshops.map(w => (
                            <option key={w.id || w._id} value={w.id || w._id}>{w.title}</option>
                          ))}
                        </select>
                      </div>
                    )}

                    <div className="flex items-center gap-2 mt-4 bg-white/15 rounded-xl px-4 py-3 backdrop-blur">
                      <Zap className="h-4 w-4 text-yellow-300 flex-shrink-0" />
                      <span className="text-sm font-bold"><span className="text-yellow-300">₹{selectedWorkshop?.price || 2999} only</span> · {seatsLeft} seats left</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-7 py-6">
                  ...
                  {selectedWorkshop ? (
                    <DynamicForm workshop={selectedWorkshop} />
                  ) : (
                    <div className="text-center py-8">
                      <Loader2 className="h-6 w-6 animate-spin mx-auto text-violet-500 mb-2" />
                      <p className="text-gray-500 text-sm">Loading workshop form...</p>
                    </div>
                  )}
                </div>
              </div>
